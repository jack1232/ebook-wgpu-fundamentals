...
fn create_vertices(rin: f32, rout: f32, h: f32, n: u16) -> (Vec<Vertex>, Vec<u16>, Vec<u16>) {
...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        let (vertex_data, index_data, index_data2) = create_vertices(0.5, 1.5, 3.0, 30);
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        if self.update_buffers {
            let (pos, _ind, _ind2) = create_vertices(self.rin, self.rout, self.h, self.n_segments);
            self.init
                .queue
        ...
        if self.recreate_buffers {
            let (pos, ind, ind2) = create_vertices(self.rin, self.rout, self.h, self.n_segments);
...
fn main() {
    ...
    window.set_title(&*format!("{}_{}", "cylinder_wireframe", sample_count));
