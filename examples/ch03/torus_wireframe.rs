...
fn create_vertices(r_torus: f32, r_tube: f32, u: u16, v: u16) -> (Vec<Vertex>, Vec<u16>, Vec<u16>) {
...
impl State {
    async fn new(window: &Window, sample_count: u32) -> Self {
        ...
        let color_buffer2 = init.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Uniform Buffer"),
        ...
        let (vertex_data, index_data, index_data2) = create_vertices(2.0, 0.5, 60, 20);
        ...
        let index_buffer2 = init.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Index Buffer"),
    ...
    fn update(&mut self, dt: std::time::Duration) {
        ...
        let mvp_ref: &[f32; 16] = mvp_mat.as_ref();
        self.init.queue.write_buffer(&self.uniform_buffers[0], 0, bytemuck::cast_slice(mvp_ref));
...
fn main() {
    ...
    window.set_title(&*format!("{}_{}", "cube_wireframe", sample_count));
